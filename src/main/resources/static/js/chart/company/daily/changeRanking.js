...
generateChangerankingChart = function (id, params) {
    changerankingUrlToData("/company/"+params.deptNo+"/day/stationTransferRank", params, id, changerankingChartOptions)
}

//根据URL请求数据，并将数据转换成绘制图表所需的格式
let changerankingUrlToData = function(url, params, id, newOptions) {
    changerankingGetSourceData(url, params, id, newOptions)
}

//根据URL获取数据
let changerankingGetSourceData = function(url,params, id, newOptions) {
    //ajax请求数据
    $.ajax({
        type: 'GET',
        url: url ,
        ...
        success: function (result) {
            ...
            //将数据嵌入到options中
            changerankingDataToOptions(data, newOptions)
            //根据id和options绘制图表
...
// 将数据嵌入到options中
let changerankingDataToOptions = function(data, options) {
    options.xAxis.data = []
