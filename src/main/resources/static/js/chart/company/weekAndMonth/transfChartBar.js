...
generateTransfChartBar = function (id, params, flag) {
    tcUrlToData("/company/"+params.deptNo+"/day/lineConsumeRank", params, id, transfChartOptions, flag)
}

//根据URL请求数据，并将数据转换成绘制图表所需的格式
tcUrlToData = function(url, params, id, newOptions, flag) {
    tcGetSourceData(url, params, id, newOptions, flag)
}

//根据URL获取数据
tcGetSourceData = function(url,params, id, newOptions, flag) {
    //根据params获取起始天数的间隔
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                result.data.forEach(item => {
                    perDayData.push({lineNo:item.lineNo,consumeCount:item.consumeCount})
                })
    ...
    //将数据嵌入到options中
    tcDataToOptions(wholeData, newOptions, timeLineData, flag)
    //根据id和options绘制图表
    generateDynamicBar(id, newOptions)
}

// 将数据嵌入到options中
tcDataToOptions = function(sData, newOptions, timeLineData, flag) {
